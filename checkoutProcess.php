...
{	
	   ...
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
		...
		$qry = "SELECT Quantity, ProductID FROM shopcartitem WHERE ShopCartID = ?";
		$stmt = $conn->prepare($qry);
		...
		if($result != null){	
			while($row = $result->fetch_array()){
				...
				$stmt2->bind_param("ii", $row["Quantity"] ,$row["ProductID"]);
				$stmt2->execute();
				$stmt2->close();
			}
		}
		...
		$qry = "UPDATE shopcart SET OrderPlaced= 1, Quantity= ?, SubTotal=?, 
									ShipCharge= ?, Tax= ?, Total= ? 
									WHERE ShopCartID = ?";
		$stmt = $conn->prepare($qry);
		// i - integer, d - double
		$stmt->bind_param("iddddi", $_SESSION['NumCartItem'], $_SESSION["SubTotal"],
							$_SESSION["ShipCharge"],$_SESSION["Tax"], $total, $_SESSION["Cart"]);
		$stmt->execute();
		$stmt->close();
		// End of To Do 2
