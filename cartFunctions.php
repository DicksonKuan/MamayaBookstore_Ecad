...
function addItem() {
	...
	if ($result->num_rows>0){
		//Selected product exsists in shopping cart
		//Increase the quantity of purchase
		$qry = "UPDATE ShopCartItem SET Quanitity = LEAST(Quanitity+?, 10) WHERE ShopCartID=? AND ProductID = ?";
		$stmt = $conn->prepare($qry);
		$stmt->bind_param("iii", $quantity, $_SESSION["Cart"], $pid); // iii - 3 integer
		$stmt->execute();
		$stmt->close();
	}
	else{
		//Selecteed prodcut has yet to be added to shopping cart
		...
		$stmt = $conn->prepare($qry);
		$stmt->bind_param("iiii", $_SESSION["Cart"], $pid, $quantity, $pid); // iii - 4 integer
		$stmt->execute();
		$stmt->close();
		$addNewItem = 1;
	}
...
function updateItem() {
	...
	$stmt = $conn->prepare($qry);
	$stmt->bind_param("iii", $quantity, $pid, $cartid); // iii - integer
	$stmt->execute();
	$stmt->close();
	$conn->close();
	header("Location: shoppingCart.php");
